...
def main(
    ...
    input_json = json.loads(open("gpt-3-v2.json", "r").read())
    for i in range(len(input_json)):
        inputs = input_json[i]
        sample = {"instruction": inputs['instruction'], "input": inputs['input']}
        ...
        print(f"Time to generate: {t:.02f} seconds.", file=sys.stderr)
        result = validate_results(output, inputs['output'])
        print(f"generated outputs {output} and expected output {inputs['output']} and result is {result}")
        input_json[i]['output_7b'] = output
        input_json[i]['result_7b'] = result
